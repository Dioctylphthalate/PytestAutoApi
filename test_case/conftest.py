...
def pytest_collection_modifyitems(items):
    ...
    appoint_items = ["test_login", "test_AddBasicInfo", "test_AddCharge", "test_BatchBilling", "test_OwnerGuidFee_all", "test_HandInRecord", "test_ArrearsReport", "test_ReceivablesReport", "test_CombinationReport", "test_PaidReport", "test_vou-set", "test_ClearTeardown"]  # test_case 文件夹中的测试用例文件运行顺序,test_ClearTeardown这个放最后，是退出登录和清除生成数据的用例
...
def pytest_terminal_summary(terminalreporter):
    ...
    try:
        _RATE = (_PASSED + _SKIPPED) / _TOTAL * 100
